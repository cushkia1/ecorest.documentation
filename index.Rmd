...
To ensure proper functionality, the `ecorest` package was rigorously assessed and peer-reviewed to ensure computational and numerical accuracy. System quality was assessed following best practices from McKay et al. (2022).
...
*Team-based coding*: Version 1.0.0 of `ecorest` was designed jointly by S.K. McKay and D.D. Hernandez-Abrams using team-based coding. Following package implementation, K.C. Cushway conducted a thorough review of model structure and accuracy by revisiting the model documentation from the USFWS for every HSI model in `ecorest` to identify potential errors and ensure code accuracy in `ecorest`. Model updates or improvements are implemented in versions 2.0.0 and 2.0.1.

*Error reporting*: To communicate issues to users and reviewers, functions in `ecorest` were designed to generate error messages in the event of incorrect inputs or user errors (refer to section 3.2 for examples). Common error messages in `ecorest` include incorrect number of input errors, out of range errors, and errors regarding incorrect formatting.

*Documentation*: Comprehensive documentation of functions in `ecorest` was auto-generated by R during publication and can be accessed at https://cran.r-project.org/web/packages/ecorest/ecorest.pdf. Documentation includes descriptions of each function, its arguments and outputs, relevant references in the literature, and example applications. In addition, the authors documented all changes made to the package during error checking, including the nature of the changes, the reasoning for updates, and confirmation of implementation. 

##### *4.1.2 Quality Control*

*Interim code checking*: During model integration into R, code functionality was checked periodically for each HSI model to ensure that models were operational, implemented correctly, and performed as expected. In addition, all models with USFWS documentation that included test data sets (n = 152 of 349) were verified for accuracy using all available test data (Appendix C). In cases where `HSIeqtn` outputs did not match the USFWS outputs, models were independently verified to determine whether mismatches were due to rounding errors, code issues in `ecorest`, or errors in USWFS documentation.

*Formal model testing*: Prior to release of `ecorest` version 2.0.0 and 2.0.1, the performance of all HSI models were tested using `SIcalc` and `HSIeqtn` to verify functionality and identify errors.

To test model performance with `SIcalc`, we implemented the following code in R:

```{r echo=TRUE, results = 'hide', eval=FALSE}

#Test SIcalc() functionality

# Load libraries
library(ecorest)

# Create a function to determine whether entire row is NA (i.e., SIV isn't present for that particular model)
all_na=function(x) all(is.na(x))

# Create a function to determine whether a specific value is not NA
not_na = function(x) sum(!is.na(x))

# Set up for loop parameters
models=data.frame(HSImetadata) # Copy HSImetadata into data frame
models.i=cbind(models,"index"=1:nrow(models))
index=models.i$index # Add an index column to models
count_NA=0 # Track the number of NA columns in a model
num_SIV=0 # Track the number of SIVs in a model
wrong_models=0 # Record models that do not perform as expected
input=c() # Store input

# Run a for loop to test whether SIcalc returns SIV scores correctly
for (i in 1:length(index)) {
  mod=HSImodels[i] # Store model
  name=names(mod[1]) # Store model name
  df=data.frame(get(name,mod)) # Pull data for model
  # Run a for loop to determine which columns in the data frame contain data, and sample data from relevant columns
  for (col in 1:length(df)) {
    if (all_na(df[,col])==TRUE){
      count_NA=count_NA+1 # If entire row is NA, number of breakpoints is NA and number of NA columns increases by 1
    }
      # For even columns:
      if (col %% 2 != 0) {
        n_rows=sum(rowSums(!is.na(df[col]))) # Find number of rows that aren't NA
        input=c(input,as.character(sample(df[1:n_rows,col],1,FALSE))) # Pull one value from the column to supply to SIcalc
        input[input == ""] = NA # Set blanks equal to NA if a column isn't used in this particular model
      }
  }
  num_SIV=(ncol(df)-count_NA)/2 # Calculate the number of input variables
  print(paste0("Model: ", name, ", index [", i, "]")) # Print the model name and index
  num_NAs = sum(is.na(SIcalc(df, input))) # Find the number of NAs
  print(paste("SIcalc outputs:", toString(SIcalc(df, input)))) # Calculate and output SIVs with SIcalc
  cat(" \n ") # Print newline
  # If the number of NA values is incorrect, stop the loop and identify the problem manually
  if (num_NAs != (count_NA/2)) {
    print('Error: something went wrong. Please revisit the number of SIV inputs.')
    stop()
  } else {
      
  }
  # Reset loop variables for next loop
  count_NA=0
  num_SIV=0
  input=c()
}
...
To test model performance with `HSIeqtn`, we tested each model 100 times with randomly generated SIV values between zero and one. Models were considered to have 'passed' testing if no errors were thrown and no outputs were NA, less than zero, or greater than one. If any model failed testing, it was re-examined and updated to fix the issue(s) causing errors.
...
i = 1 # Iterator
count_NA = 0 # Vector to store number of NA columns in model
num_SIV = 0 # Vector to store number of SIV variables in model
wrong_models = 0 # Vector to store models that aren't working properly

models = data.frame(HSImetadata) # Store model information in data frame
models.i = cbind(models,"index" = 1:nrow(models)) # Add index column
index = models.i$index # Store index as a vector
mat = matrix(NA, nrow = length(index), ncol = 100 + 1) # Create a matrix to store outputs
mat[, 1] = HSImetadata[, 1] # Assign model names to first column of matrix

# Run each model 100 times using different random SIV inputs between 0 and 1
for (s in 1:100) {
  for (i in 1:length(index)) {
    mod = HSImodels[i] # Store model
    name = names(mod[1]) # Store model name
    df = data.frame(get(name,mod)) # Retrieve model data and add to data frame
    for (col in 1:length(df)) {
      if (all_na(df[,col]) == TRUE){
    ...
    num_SIV = (ncol(df)-count_NA)/2 # Calculate the number of input variables
    input = runif(num_SIV,0,1) # Generate random test input from a uniform distribution between 0 and 1
    x = try(HSIeqtn(name,input,HSImetadata)) # Calculate overall HSI if possible
    
    if (x < 0 | x > 1){
      print("Error: HSI value outside of valid range.")
      stop()
    } else if (is.na(x)) {
      print("Error: HSI value not calculated. Please review SIV inputs.")
    } else {
      mat[i, s+1] = x # Add result to matrix
    }
