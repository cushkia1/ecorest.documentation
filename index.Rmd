...
To test model performance with `SIcalc`, we implemented the following code in R. Models were considered to have 'passed' testing if no errors were produced and no outputs were less than zero or greater than one.
...
Models are all literature-based and incorporate expert opinion

Sensitivity and uncertainty analysis conducted for every model - summarise this and add link to PDF summaries on GitHub

Discuss how users should critically evaluate whether models are applicable in their current state, and could consider running an additional sensitivity analysis using code in Appendix D if uniform distribution doesn't seem reasonable.
